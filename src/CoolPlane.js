...
function CoolPlane( {data} ) {
  ...
  function onMouseMove(e) {
    ...
    let face = e.intersections[0].face
    console.log("e.intersections[0]", e.intersections[0])
    ...
    setColorArray2(prev => {
      let newState = prev
      newState[face.a*3] = 0.1 
      newState[face.b*3] = 0.1 
      newState[face.c*3] = 0.1 
  ...
  const updateBufferGeometry = () => {
    ...
    for (let i = 0; i < position.array.length; i += 3) {
      ...
      position.array[i+2] = z + 2*Math.random()
  ...
  useEffect(() => updateBufferGeometry(), []);
  
  const colors = []
  function fillColors() {
    for (let i = 0; i < 9828; i++)
      colors.push(1, 0, 0)
  ...
  return (
    ...
    <mesh ref={meshRef} onPointerMove={e => onMouseMove(e)}>
      ...
      <meshPhongMaterial ref={matRef} attach="material" vertexColors={true} side={THREE.DoubleSide} flatShading={THREE.FlatShading} />
    </mesh>
    </>
  )
}
